...
abstract class TweetSet {
  ...
  /**
    ...
    *
    * We have to implement this method in the subclass in order to have access to the left, right and element
    *
    */
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = this.mostRetweetedAcc(elem, elem)

  def mostRetweetedAcc(next: Tweet, most: Tweet): Tweet = {
    val leftMost = left.mostRetweeted
    val rightMost = right.mostRetweeted
    val topMost = if(next.retweets > most.retweets ) next else most
    if(leftMost == null && rightMost == null) {
      topMost
    } else if(leftMost != null && rightMost == null){
      if(leftMost.retweets > topMost.retweets) leftMost else topMost
    } else if (leftMost == null && rightMost != null) {
      if(rightMost.retweets > topMost.retweets) rightMost else topMost
    } else {
      val leafMost = if(rightMost.retweets > leftMost.retweets) rightMost else leftMost
      if (leafMost.retweets > topMost.retweets) leafMost else topMost
    }
  }
